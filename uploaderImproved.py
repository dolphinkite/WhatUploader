...
--- functions:
    ...
    create_torrent(folder,tracker) -- creates a torrent file (using the
        ...
        is running from. If a 3rd argument is passed to uploader.py, it will be used
        as the output path for the newly created .torrent
...
--- command line arguments (optional)
	...
	sys.argv[3] -- torrentFolder, specifies where to save the newly created .torrent files
...
import urllib, urllib2 as req, os, threading, time, sys, re
...
## If set to True, only the inFolder will be considered
debug = True       ## displays full traceback instead of short error

if debug:
    #sys.argv = ['d:/Software/uploader-master/uploaderImproved.py', 'D:\\temp\\rip2\\Method\ Man\ -\ Tical\ [Reissue]\ (1994)\ [FLAC]', 'D:/temp/tt']
    #sys.argv = [re.escape('d:\Software\uploader-master\uploaderImproved.py'), re.escape('D:\temp\rip2\test'), re.escape('D:\temp\tt')]
    sys.argv = ['d:/Software/uploader-master/uploaderImproved.py', 'D:/temp/rip2/test', 'D:/temp/tt']
...
#pprint('Flac path='+flac_path)
lame_path = os.path.join(curdir,lame_path)
#print('Lame path='+lame_path)
...
def create_torrent(inFolder=None,outFolder=None,tracker=None):
    if not isinstance(inFolder,str):
        inFolder = str(inFolder)
        
    if not isinstance(outFolder,str):
            outFolder = str(outFolder)
    ct.main(inFolder,tracker)
##    torrent_filename = '%s/%s.torrent' % (folder, folder.split('/')[-1])
    torrent_filename = '%s.torrent' % (os.path.split(folder)[1])
    pprint('Torrent created: '+torrent_filename)
...
def get_tracker_url(url='https://what.cd/upload.php'):
    ...
    tracker_url = '%s%s' % ('http://tracker.what.cd', data.split('http://tracker.what.cd')[1].split('"')[0]) 
...
def to_imgur(filename):
    ...
    data = req.urlopen(imgurl,data={'image':open(filename,'rb'),'key':key}).read()
...
def get_extra_data(addformat=False):
    if addformat:
        ...
        if releaseTypeConst in globals():
            release_type = releaseTypeConst
        else:
...
<option value='21'> Unknown </option>\n\n>>> ''')
        ...
        if additionalDescripton:
            desc = raw_input('Description (in addition to the auto-generated track listing) (optional):\n\n>>> ')
        else:
            desc = ''

        if mediaConst in globals():
            media=mediaConst
        else:
...
def run(inFolder=None, torrentFolder=curdir, reuse_extras=False,argv=None):
    
    authkey = get_auth()
    tracker_url = get_tracker_url()
    torrent = create_torrent(inFolder, torrentFolder, tracker_url)
    tags = get_tags(inFolder)
    extras = get_extra_data()
    data = create_data(torrent, tags, extras, authkey)
    if noUpload:
        pprint('Not uploading')
...
def run_addformat(inFolder=None,groupid=None,extras=None):
    if not inFolder:
        inFolder = select_folder('Select the folder to be uploaded')

    authkey = get_auth()
    tracker_url = get_tracker_url()
    torrent = create_torrent(inFolder, tracker_url)
    tags = get_tags(inFolder,addformat=True)
...
def get_number_of_processors():
    try:        # python 2.6+
        ...
        pass
    try:        # windows
        procNumber = int(os.environ['NUMBER_OF_PROCESSORS'])
        if procNumber > 0:
            return procNumber
    except (KeyError, ValueError):
        pass
    try:        # linux
        procNumber = open('/proc/cpuinfo').read().count('processor\t:')
        if procNumber > 0:
            return procNumber
    except IOError:
        pass
...
def get_threads_number():
    #returns the number of processors, plus the number of currently running threads.
    current_threads = actives()
...
def make_lame_metadata(artist,title=None,album=None,year=None,track_num=None):
    if not isinstance(artist,str):
        ...
        track_num = artist[4]
        artist = artist[0]
    string = '--ta "%s" --tt "%s" --tl "%s" --ty "%s" --tn "%s"' % (artist,title,album,year,track_num)
    return string
...
##    if not isinstance(flac_path,str):
##       flac_path = str(flac_path)            

    flacCommand = '"'+ flac_path + '"' + ' -d -s -c "' + file_in + '"'
    flacCommand2 = '%s -d -s -c "%s"' % (flac_path, file_in)
    pprint(flacCommand)
    pprint(flacCommand2)
    
    decoder = os.popen(flacCommand2, 'rb', -1)
    
    lameCommand = '"' + lame_path + '"' + ' --silent ' + lame_params + ' - -o "' + file_out + '.mp3" ' + lame_metadata
    lameCommand2 = '%s --silent %s - -o "%s.mp3" %s' % (lame_path, lame_params, file_out, lame_metadata)
    
    pprint(lameCommand)
    pprint(lameCommand2)
    
    encoder = os.popen(lameCommand2, 'wb', -1)
...
def convert_perfect_three(inFolder=None,outfolder=None,torrentFolder=None):
    ...
    logs_cues = []  #same. We won't copy those in the mp3 folders
    etc = []        #same. We will copy those in the mp3 folders
    for root, dirs, filenames in os.walk(inFolder):
        for filename in filenames:
            abspath = os.path.join(root,filename)
            if filename.endswith('.flac'):
                flacs.append((abspath,relpath(abspath,inFolder),filename))
                pprint(filename+' in flac')
            elif filename.endswith('.log') or filename.endswith('.cue'):
                logs_cues.append((abspath,relpath(abspath,inFolder),filename))
                pprint(filename+' in log and cues')
            else:
                etc.append((abspath,relpath(abspath,inFolder),filename))
                pprint(filename+' in etc')
    #get metadata
    ...
    for f in flacs:
        ...
        pprint('reldir='+reldir)         #for debugging
...
def perfect_three_and_upload(inFolder=None,outfolder=None,torrentFolder=None):
    
    converted = convert_perfect_three(inFolder,outfolder,torrentFolder)
    print(converted)              ## for debugging

    ##This is where the upload of the FLAC file happens
    groupid,extras = run(inFolder,reuse_extras=True)    # da flac = main upload

    ##This is where the upload of the mp3 files happen
    for f in converted:
...
if __name__ == "__main__": 
    ...
    if len(sys.argv) > 2:
        ...
        outfolder = None
    if len(sys.argv) > 3:
        torrentFolder = sys.argv[3]
    else:
        torrentFolder = None
    
    pprint('Folder to convert: '+folder)
    pprint('Arguments: '+str(sys.argv))           # for debugging

    try:
        globals()[main_action](folder,outfolder,torrentFolder)
